...
- **Always default to the latest LTS (Long Term Support) version** of any runtime, framework, or platform unless explicitly told otherwise
- Before recommending a version, verify it is the current LTS — never assume based on training data (use Context7 or release notes)
- Flag when a newer LTS has superseded a previously recommended version
- If no LTS exists for a given tool, use the latest stable release
- Never recommend a STS (Standard Term Support) or preview version as a default

## 📚 Documentation & Library References
- **Always use Context7 (`mcp__plugin_context7_context7`) for every reference to any technology, library, framework, SDK, API, runtime, or tool** — no exceptions
- This applies to: recommending a stack, verifying a version, checking an API, confirming compatibility, reviewing setup instructions, or answering any question that touches a specific technology
- Never rely on internal training data for technology-specific facts — always fetch from Context7 first, then answer
- If Context7 returns no results for a library, state that explicitly rather than falling back to internal knowledge silently
- Run `resolve-library-id` first, then `query-docs` — do not skip the resolve step
- Verify every component in a recommended stack individually — do not assume a stack is correct because some components were verified
...
Every new endpoint, service method, and data-access method must be evaluated against these controls **and all OWASP Top 10 risks**. Flag any gap in the plan phase before writing code.

### 🛡️ OWASP Top 10 Compliance
All code must be evaluated against the current OWASP Top 10. Flag violations in the plan phase before writing code.

| # | Risk | Requirement |
|---|---|---|
| A01 | **Broken Access Control** | Enforce authorization on every resource. Deny by default. Never rely on obscurity or client-side checks. |
| A02 | **Cryptographic Failures** | Never roll your own crypto. Use vetted libraries. Encrypt sensitive data at rest and in transit. Never use MD5/SHA1 for security purposes. |
| A03 | **Injection** | Parameterized queries only. Never concatenate user input into queries, commands, or expressions. |
| A04 | **Insecure Design** | Threat-model new features. Apply defense-in-depth. Do not rely on a single security control. |
| A05 | **Security Misconfiguration** | No default credentials, verbose error pages, or unnecessary features enabled. Harden all environments. |
| A06 | **Vulnerable & Outdated Components** | Flag use of known-vulnerable dependencies. Prefer actively maintained libraries. |
| A07 | **Identification & Authentication Failures** | Enforce strong auth, rate-limit login attempts, never expose session tokens, invalidate sessions on logout. |
| A08 | **Software & Data Integrity Failures** | Verify integrity of packages and pipelines. Never deserialize untrusted data without validation. |
| A09 | **Security Logging & Monitoring Failures** | Log all auth events, access control failures, and input validation errors. Never log sensitive data. |
| A10 | **Server-Side Request Forgery (SSRF)** | Validate and allowlist all outbound URLs. Never forward raw user-supplied URLs to internal services. |
